    ...
    "recording_action_to_cache": [
        {
            ...
            "instructions": [
                ...
                "For pointer clicks (click, double_click, right_click, middle_click, hold): return only the move-target portion starting with 將滑鼠移到. Do not include nearby comments or the trailing action phrase; post-processing adds those (including Ctrl+/Shift+ click phrasing when RecordedEvent.modifiers is set, and 按住約N秒 / 用右鍵按住約N秒 when kind=hold using duration_seconds). If the click is on the anchor, e.g. 將滑鼠移到「Submit」按鈕, 將滑鼠移到「間間Gemini」文字所在的輸入欄, 將滑鼠移到輸入欄, 將滑鼠移到「資產總覽」文字區域的滾動條, or 將滑鼠移到滾動條. If the click is off the anchor, append the relative pixel offset from the offset hints, e.g. 將滑鼠移到「自訂Office 範本」文字下方39個像素的位置. Use the exact pixel counts from the hints. Do not use vague 附近 for the primary target when an offset is available.",
