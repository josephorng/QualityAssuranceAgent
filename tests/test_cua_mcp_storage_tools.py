...
from cua_mcp.tool_module import _screenshot, _store_clipboard_text, _store_image, _store_text
...
def test_store_clipboard_text_writes_file_and_indexes(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setenv("CUA_RUN_ROOT", str(tmp_path / "run_clip"))

    monkeypatch.setattr(
        "cua_mcp.storage.pyperclip.paste",
        lambda: "copied snippet",
    )

    result = _store_clipboard_text(title="from clip", tags=["paste"], file_name="clip.txt")
    entry = result["entry"]
    ...
    assert result["status"] == "stored"
    assert entry["type"] == "text"
    assert entry["source"] == "clipboard"
    assert entry["text"] == "copied snippet"
    assert stored_path.read_text(encoding="utf-8") == "copied snippet"
    assert stored_path.parent == storage_root / "storage"
    assert stored_path.name == "clip.txt"
    assert isinstance(rows, list)
