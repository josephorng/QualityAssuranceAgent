...
from cua_mcp.read_screen_text import get_coordinates as get_coordinates_mod
...
def test_offset_region_shifts_bbox_and_center() -> None:
    ...
    offset = get_coordinates_mod._offset_region(region, 1920, 0)
...
def test_get_coordinates_from_selected_monitors(
    ...
    monkeypatch.setattr(get_coordinates_mod, "selected_eye_monitor_indices", lambda: [1, 2])
    monkeypatch.setattr(get_coordinates_mod, "capture_monitor_to_file", _fake_capture)
    monkeypatch.setattr(get_coordinates_mod, "get_coordinates_from_image_path", _fake_ocr)
    monkeypatch.setattr(get_coordinates_mod, "active_monitor_offset", lambda idx: offsets[idx])
    monkeypatch.setattr(get_coordinates_mod, "ts_name", lambda: "20260610_test")
    monkeypatch.setattr(get_coordinates_mod, "get_run_state_manager", lambda: manager)

    regions, image_paths = get_coordinates_mod.get_coordinates_from_selected_monitors()
